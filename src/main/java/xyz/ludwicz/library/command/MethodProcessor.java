    ...
    public Set<CommandNode> process(Method value) {
        if (value.isAnnotationPresent(Command.class) && value.getParameterCount() >= 1 && CommandSender.class.isAssignableFrom(value.getParameterTypes()[0])) {
            ...
            for ( String name : command.names() ) {
                ...
                if (LudwigCommandHandler.ROOT_NODE.hasCommand(primary)) {
                    workingNode= LudwigCommandHandler.ROOT_NODE.getCommand(primary);
                    change=false;
                }
                if (change) {
                ...
                LudwigCommandHandler.ROOT_NODE.registerCommand(workingNode);
